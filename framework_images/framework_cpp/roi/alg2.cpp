...
int processImage(Mat &src) {
	int threshhold_val;
	Mat hsv, eroded, final, corners, corners_norm, edges;
	...
	//erode(hsv, eroded, erosion_element);
	...
	for(int i = 0; i < contours.size(); i++) {
		approxPolyDP(Mat(contours[i]),
				...
				true);
		if (fabs(contourArea(contours[i])) < 150 || fabs(contourArea(contours[i])) > 500|| !isContourConvex(approx))
			continue;
		if(approx.size() == 8){
			return 1; //stop detected
		}
	}
	return 0;
	/*
